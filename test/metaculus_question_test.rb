...
class MetaculusQuestionTest < Minitest::Test
  ...
  def test_criteria_content_handles_nil_fine_print
    q = Metaculus::Question.new(data: question_data(extra_question: { 'fine_print' => nil }))
  ...
  def test_continuous_cdf_returns_201_values
    result = make_numeric.continuous_cdf(SAMPLE_PERCENTILES.dup)
    assert_equal 201, result.length
  end

  def test_continuous_cdf_values_in_unit_interval
    result = make_numeric.continuous_cdf(SAMPLE_PERCENTILES.dup)
    result.each { |v| assert v >= 0.0 && v <= 1.0, "CDF value #{v} outside [0, 1]" }
  end

  def test_continuous_cdf_is_monotonically_nondecreasing
    result = make_numeric.continuous_cdf(SAMPLE_PERCENTILES.dup)
    result.each_cons(2) { |a, b| assert a <= b + 1e-10, "CDF decreased: #{a} -> #{b}" }
  end
  ...
  def test_continuous_cdf_closed_bounds_starts_at_zero
    result = make_numeric(open_lower: false, open_upper: false).continuous_cdf(SAMPLE_PERCENTILES.dup)
  ...
  def test_continuous_cdf_open_lower_starts_above_zero
    result = make_numeric(open_lower: true, open_upper: false).continuous_cdf(SAMPLE_PERCENTILES.dup)
    assert result.first > 0.0, "Expected first CDF value > 0 for open lower bound"
  end

  def test_continuous_cdf_open_upper_ends_below_one
    result = make_numeric(open_lower: false, open_upper: true).continuous_cdf(SAMPLE_PERCENTILES.dup)
  ...
  def test_continuous_cdf_both_open_starts_above_zero_ends_below_one
    result = make_numeric(open_lower: true, open_upper: true).continuous_cdf(SAMPLE_PERCENTILES.dup)
    assert result.first > 0.0, "Expected first CDF value > 0 for open lower bound"
    assert result.last  < 1.0, "Expected last CDF value < 1 for open upper bound"
  end

  def test_continuous_cdf_clamps_values_near_closed_boundary
    # Percentiles at the exact range bounds should be buffered inward, not crash
    boundary_percentiles = SAMPLE_PERCENTILES.merge(5 => 0.0, 95 => 100.0)
    result = make_numeric(open_lower: false, open_upper: false).continuous_cdf(boundary_percentiles)
    assert_equal 201, result.length
    result.each_cons(2) { |a, b| assert a <= b + 1e-10 }
  end

  def test_continuous_cdf_large_range
    q = make_numeric(open_lower: false, open_upper: false, range_min: 0.0, range_max: 10_000.0)
    pcts = {
  ...
  def test_aggregate_content_binary_shows_forecaster_count_and_median
    result = make_question(type: 'binary').aggregate_content
  ...
  def test_aggregate_content_returns_empty_string_when_no_aggregations
    ...
    assert_equal '', Metaculus::Question.new(data: data).aggregate_content
  end
end
